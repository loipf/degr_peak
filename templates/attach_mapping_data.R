...
gene_info = fread("mapping_data.txt") %>%
  ...
  .[, .(ENTREZID, PROBESET, CHR_Mapping, BP_Mapping)] #required columns
...
for (chromosome in CES_data_with_mapping[, unique(CHR_Mapping)]){
  fwrite(CES_data_with_mapping[CHR_Mapping == chromosome, ], glue("peaks_with_mapping_{chromosome}.txt"), sep = "\t")
}
